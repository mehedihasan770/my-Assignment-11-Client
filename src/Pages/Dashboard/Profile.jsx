...
const Profile = () => {
  ...
  const winRate =
    ParticipatedContests > 0
      ? Math.round(
          (WiningContests?.length / ParticipatedContests?.length) * 100
        )
      : 0;
  ...
  const adminData = [{ name: "Total User", value: totalUser }];

  const creatorData = [{ name: "My Created Contest", value: totalContest }];

  const COLORS = ["#34D399", "#F87171"];

  if (loading || isLoading || loading1 || roleLoading || loading2 || loading3 || loading4)
    return <Loader />;

  return (
    <div>
      <div className="mx-auto   dark:bg-[#261B25]  bg-secondary p-8 rounded-2xl shadow-xl overflow-hidden">
        ...
        <div className="w-full h-80 rounded-2xl relative">
          ...
          <img
            src={user?.photoURL}
            alt="Profile"
            className="w-32 h-32 rounded-full border-4 border-primary absolute -bottom-14 left-1/2 transform -translate-x-1/2 shadow-xl object-cover"
          />
        </div>

        <div className="mt-20 text-center">
          <h2 className="text-3xl font-bold text-gray-5 tracking-wide">
            {user?.displayName}
          </h2>
          <p className="text-gray-600 dark:text-gray-500 mt-1 text-[15px]">
            {user?.email}
          </p>
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-2 text-sm">
            Bio: <span className="font-medium">{profile.bio}</span>
          </p>
        </div>
          ...
          <dialog id="my_modal_1" className="modal">
            <div className="modal-box">
              <form
                ...
                className=" dark:bg-gray-900 p-6 rounded-2xl shadow-md w-full max-w-md mx-auto"
              >
                <h2 className="text-2xl font-semibold mb-6 text-center">
                  Update Profile
                </h2>
                <div className="mb-4">
                  <label className="block text-gray-500 dark:text-gray-300 mb-1 font-medium">
                    Name
                  </label>
                  <input
                    type="text"
                    ...
                    className="w-full border-primary px-4 py-2 rounded-lg border dark:border-gray-700 dark:bg-gray-800 focus:ring-2 focus:ring-primary focus:outline-none"
                  />
                  {errors.name && (
                ...
                <button className="btn w-full py-2 bg-primary text-white rounded-xl font-medium hover:bg-primary/90 transition">
                  Save Changes
                </button>
              </form>
      ...
      {roleData?.role === "user" ? (
        <div className="w-full shadow-xl mt-5   dark:bg-[#261B25]  bg-secondary p-6 rounded-2xl">
          <h2 className="text-xl font-semibold text-gray-500 dark:text-white mb-4 text-center">
            Win Percentage
          </h2>
          ...
          <div className="mt-4 text-center">
            ...
            <p className="text-gray-500 dark:text-gray-300">
              Total Winning:{" "}
              <span className="font-semibold">{WiningContests?.length}</span>
            </p>
            <p className="text-gray-500 dark:text-white text-lg font-bold mt-2">
              Win Rate: {winRate}%
            </p>
          </div>
        </div>
        ...
        <div className="w-full shadow-xl mt-5   dark:bg-[#261B25]  bg-secondary p-6 rounded-2xl">
          ...
          <div className="mt-4 text-center">
            <p className="text-gray-500 dark:text-white text-lg font-bold mt-2">
              Created Contest: {contests?.length}
            </p>
          </div>
        </div>
      )}
